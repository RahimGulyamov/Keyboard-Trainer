...
import pyjokes
import time
from src.attributes import Attributes as self
...
class Game:
    ...
    We use global parameters from attributes.py
    ...
    def display(header_text, header_text_2, text, finish_key, rect_size,
        ...
        # Q2 = key_font.render('Q', True, 'black')
        # Q2_xy = Q2.get_rect(center=(120, 620))
        #
        # pg.draw.rect(self.screen, 'white', (100, 600, 40, 40))
        # pg.draw.rect(self.screen, 'black', (100, 600, 40, 40), 3)
        # self.screen.blit(Q2, Q2_xy)

        key_font = pg.font.SysFont('mannerliness', self.text_size)

        x = self.keyboard.x_keys_from_1
        y = self.keyboard.y_keys_from_1
        for key in self.keyboard.keys_from_1:
            key_letter = key_font.render(key.letter, True, 'black')
            key_xy = key_letter.get_rect(center=(x + self.keyboard.width / 2, y + self.keyboard.length / 2))
            pg.draw.rect(self.screen, key.color, (x, y, self.keyboard.width, self.keyboard.length))
            pg.draw.rect(self.screen, 'black', (x, y, self.keyboard.width, self.keyboard.length), 3)
            self.screen.blit(key_letter, key_xy)
            x += (self.keyboard.width + self.keyboard.space)

        x = self.keyboard.x_keys_from_Q
        y = self.keyboard.y_keys_from_Q
        for key in self.keyboard.keys_from_Q:
            key_letter = key_font.render(key.letter, True, 'black')
            key_xy = key_letter.get_rect(center=(x + self.keyboard.width / 2, y + self.keyboard.length / 2))
            pg.draw.rect(self.screen, key.color, (x, y, self.keyboard.width, self.keyboard.length))
            pg.draw.rect(self.screen, 'black', (x, y, self.keyboard.width, self.keyboard.length), 3)
        ...
        x = self.keyboard.x_keys_from_A
        y = self.keyboard.y_keys_from_A
        for key in self.keyboard.keys_from_A:
            key_letter = key_font.render(key.letter, True, 'black')
            key_xy = key_letter.get_rect(center=(x + self.keyboard.width / 2, y + self.keyboard.length / 2))
            pg.draw.rect(self.screen, key.color, (x, y, self.keyboard.width, self.keyboard.length))
            pg.draw.rect(self.screen, 'black', (x, y, self.keyboard.width, self.keyboard.length), 3)
        ...
        x = self.keyboard.x_keys_from_Z
        y = self.keyboard.y_keys_from_Z
        for key in self.keyboard.keys_from_Z:
            key_letter = key_font.render(key.letter, True, 'black')
            key_xy = key_letter.get_rect(center=(x + self.keyboard.width / 2, y + self.keyboard.length / 2))
            pg.draw.rect(self.screen, key.color, (x, y, self.keyboard.width, self.keyboard.length))
            pg.draw.rect(self.screen, 'black', (x, y, self.keyboard.width, self.keyboard.length), 3)
        ...
        x = self.keyboard.x_key_space
        y = self.keyboard.y_key_space
        key_letter = key_font.render(self.keyboard.key_space.letter, True, 'black')
        key_xy = key_letter.get_rect(center=(x + 250 / 2, y + self.keyboard.length / 2))
        pg.draw.rect(self.screen, self.keyboard.key_space.color, (x, y, 250, self.keyboard.length))
        pg.draw.rect(self.screen, 'black', (x, y, 250, self.keyboard.length), 3)
        self.screen.blit(key_letter, key_xy)
